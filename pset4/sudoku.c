...
main(int argc, char *argv[])
{
    ...
    //create a bidimensional array with the old values for reference
    int array[9][9];
        ...
        switch (ch)
	{
	    ...
	    // moves the cursor up, skipping the grid
	    case KEY_UP:
		if (g.y == 0)
		{
		    move(g.top + g.y + 10 + g.y/3, g.left + 2 + 2*(g.x + g.x/3));
		    g.y = 8;
		}
		...
		{
		    move(g.top + g.y + g.y/3, g.left + 2 + 2*(g.x + g.x/3));
		    g.y--;
		}
		break;

	    // moves the cursor down, skipping the grid
	    case KEY_DOWN:
		if (g.y == 8)
		{
		    move(g.top + g.y - 9 + g.y/3, g.left + 2 + 2*(g.x + g.x/3));
		    g.y = 0;
		}

		else if (g.y == 2 || g.y == 5)
		{
		    move(g.top + 3 + g.y + g.y/3, g.left + 2 + 2*(g.x + g.x/3));
		    g.y++;
		}

		else
		{
		    move(g.top + 2 + g.y + g.y/3, g.left + 2 + 2*(g.x + g.x/3));
		    g.y++;
		}
		break;

	    // moves the cursor left, skipping the grid
	    case KEY_LEFT:
		if (g.x == 0)
		{
		    move(g.top + g.y + 10 + g.y/3, g.left + 2 + 2*(g.x + g.x/3));
		...
		{
		    move(g.top + 1 + g.y + g.y/3, g.left - 2 + 2*(g.x + g.x/3));
		    g.x--;
		}

		else
		{
		    move(g.top + 1 + g.y + g.y/3, g.left + 2*(g.x + g.x/3));
		    g.x--;
		}
		break;

	     // moves the cursor right, skipping the grid
	     case KEY_RIGHT:
		if (g.x == 8)
		{
		    move(g.top + g.y - 9 + g.y/3, g.left + 2 + 2*(g.x + g.x/3));
		...
		{
		    move(g.top + 1 + g.y + g.y/3, g.left + 6 + 2*(g.x + g.x/3));
		    g.x++;
		}

		else
		{
		    move(g.top + 1 + g.y + g.y/3, g.left + 4 + 2*(g.x + g.x/3));
		    g.x++;
		}
		break;

	    // allows fill (only) the empty spaces with 1 if the game wasn't won
	    case '1':
		if (array[g.y][g.x] == 0 && !win())
		{
		    g.board[g.y][g.x] = 1;
		    draw_numbers();
		    show_cursor();
	    ...
	    // allows fill (only) the empty spaces with 2 if the game wasn't won
	    case '2':
		if (array[g.y][g.x] == 0 && !win())
		{
		    g.board[g.y][g.x] = 2;
	    ...
	    // allows fill (only) the empty spaces with 3 if the game wasn't won
	    case '3':
		if (array[g.y][g.x] == 0 && !win())
		{
		    g.board[g.y][g.x] = 3;
	    ...
	    // allows fill (only) the empty spaces with 4 if the game wasn't won
	    case '4':
		if (array[g.y][g.x] == 0 && !win())
		{
		    g.board[g.y][g.x] = 4;
	    ...
	    // allows fill (only) the empty spaces with 5 if the game wasn't won
	    case '5':
		if (array[g.y][g.x] == 0 && !win())
		{
		    g.board[g.y][g.x] = 5;
	    ...
	    // allows fill (only) the empty spaces with 6 if the game wasn't won
	    case '6':
		if (array[g.y][g.x] == 0 && !win())
		{
		    g.board[g.y][g.x] = 6;
	    ...
	    // allows fill (only) the empty spaces with 7 if the game wasn't won
	    case '7':
		if (array[g.y][g.x] == 0 && !win())
		{
		    g.board[g.y][g.x] = 7;
	    ...
	    // allows fill (only) the empty spaces with 8 if the game wasn't won
	    case '8':
		if (array[g.y][g.x] == 0 && !win())
		{
		    g.board[g.y][g.x] = 8;
	    ...
	    // allows fill (only) the empty spaces with 9 if the game wasn't won
	    case '9':
		if (array[g.y][g.x] == 0 && !win())
		{
		    g.board[g.y][g.x] = 9;
	    ...
	    // allows change an entered number to a blank space if the game wasn't won
	    case '0':
		if (array[g.y][g.x] == 0 && !win())
		{
		    g.board[g.y][g.x] = 0;
	...
	// chek if the game has been won or has errors
	errors();
	win();

	char *a = "Número inválido!";
	char *b = "Parabéns, você venceu o jogo!";

	// show a banner that warns of errors
	if (!errors())
	{
	    show_banner(a);
	    show_cursor();
	...
	// if the error has been corrected, the banner disappears
	else
	{
	    hide_banner();
	    show_cursor();
	...
	// show a banner that congratulates the user if the game has been won
	if (win())
	{
	    show_banner(b);
	    show_cursor();
...
errors(void)
{
    // check for repeated numbers in the same column
    for (int i = 0; i < 9; i++)
    {
	for (int j = 0; j < 9; j++)
	{
	    ...
	    for (int k = 0; k < 9; k++)
	    {
		if (a == g.board[i][k] && a != 0 && k != j)
		    return false;
	    }
	}
    }

    // check for repeated numbers in the same line
    for (int i = 0; i < 9; i++)
    {
	for (int j = 0; j < 9; j++)
	{
	    ...
	    for (int k = 0; k < 9; k++)
	    {
		if (a == g.board[k][i] && a != 0 && k != j)
		    return false;
	    }
	}
    }

    // check for repeated numbers in the same 3x3 box
    for (int i = 0; i < 9; i++)
    {
	for (int j = 0; j < 9; j++)
	{
...
startup(void)
{
    ...
    if (has_colors())
    {
	    ...
	    init_pair(PAIR_DEFAULT, FG_DEFAULT, BG_DEFAULT) == ERR)
        {
            endwin();
            return false;
        }
...
win(void)
{
    // the game is only won when there aren't errors
    if(!errors())
	return false;

    // the game is only won all spaces are filled
    for (int i = 0; i < 9; i++)
    {
	for (int j = 0; j < 9; j++)
	{
